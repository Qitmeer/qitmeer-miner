...
bool meer_drv_init(int *fd, int num_chips)
{
    ...
    *fd = fdtemp;
    ...
    usleep(100000);
    uart_write_register(fdtemp,0x90,0x00,0x00,0x82,DEF_SLOT_1M*num_chips);	//配芯片发送总时隙
    usleep(10000);
...
bool meer_drv_set_work(int fd, struct work *work, int num_chips)
{
    ...
    uart_write_register(fd,0x44,0x00,chip_id,0x41, 0x00000040);
    uart_write_register(fd,0x44,0x00,chip_id,0x42, 0x01);
	...
	uart_write_register(fd,0x44,0x00,chip_id,0x3f,force_start); //
    group_id = 1;
    if(group_id < 2) {
        pstart[0] |= (1<<(group_id+6));
    } else {
