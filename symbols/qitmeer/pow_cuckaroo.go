...
func (this *Cuckaroo) Mine(wg *sync.WaitGroup) {
	...
	for {
		...
		for {
			...
			if pow.CalcCuckooDiff(pow.GraphWeight(uint32(this.EdgeBits),int64(this.header.Height),this.Cfg.NecessaryConfig.Param.PowConfig.BigGraphStartHeight,pow.CUCKAROO),h).Cmp(this.header.TargetDiff) < 0{
				continue
			}
