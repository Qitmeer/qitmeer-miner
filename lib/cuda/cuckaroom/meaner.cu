...
__global__ void EdgesMeet(uint2 *source, u32 *sourceIndexes, u32 *destIndexes)
{
  ...
  for (int a = 0; a < NMEM; a++) {
    ...
    for (int i = 0; i < npairs[a]; i++) {
      ...
      int j = atomicAdd(destIndexes + bucket, 1);
      const u32 dlt = (bucket % NB_NMEM) * NMEM * maxIn/2;
      source[delta2 + dlt + j] = pair;
    }
  }
}
...
struct edgetrimmer {
  ...
  edgetrimmer(const trimparams _tp) : tp(_tp) {
    ...
    if (checkCudaErrors_V(cudaMalloc((void**)&indexesA, indexesSizeNTLB)) ) return ;
    if (checkCudaErrors_V(cudaMalloc((void**)&indexesB, indexesSizeNMEM + indexesSize))) return ;
    if (checkCudaErrors_V(cudaMalloc((void**)&edgemap, edgemapSize))) return ;
    if (checkCudaErrors_V(cudaMalloc((void**)&bufferB, bufferSize))) return ;
  ...
  ~edgetrimmer() {
    if (checkCudaErrors_V(cudaFree(bufferB)))return ;
    if (checkCudaErrors_V(cudaFree(indexesA)))return ;
    if (checkCudaErrors_V(cudaFree(indexesB)))return ;
    if (checkCudaErrors_V(cudaFree(edgemap)))return ;
...
CALL_CONVENTION SolverCtx* create_solver_ctx(SolverParams* params) {
  ...
  cudaDeviceProp prop;
  if (checkCudaErrors_N(cudaGetDeviceProperties(&prop, params->device))) return NULL;
  ...
  if (!params->cpuload)
   if( checkCudaErrors_N(cudaSetDeviceFlags(cudaDeviceScheduleBlockingSync)) ) return NULL;
