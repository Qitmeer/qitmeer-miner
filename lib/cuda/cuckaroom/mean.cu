...
struct edgetrimmer {
  ...
  edgetrimmer(const trimparams _tp) : tp(_tp) {
    ...
    if (checkCudaErrors_V(cudaMalloc((void**)&indexesA, indexesSizeNA)) ) return;
    if (checkCudaErrors_V(cudaMalloc((void**)&indexesB, indexesSizeNA)) ) return;
    if (checkCudaErrors_V(cudaMalloc((void**)&nodemap, nodemapSize)) ) return;
  ...
  ~edgetrimmer() {
    ...
    if (checkCudaErrors_V(cudaFree(nodemap)) ) return;
